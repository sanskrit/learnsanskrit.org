...
TABLE_OF_CONTENTS = [
    ...
    Page(
        ...
        children=[
            ...
            Page(
                ...
                children=[
                    Page(
                        "/nouns/pronouns/short", "Short Pronouns, etad, and ena", True
                    ),
        ...
        children=[
            ...
            Page(
                ...
                children=[
                    ...
                    Page(
                        "/verbs/doubling/distant-2",
                        "Distant Past Tense 2: Irregular Verbs",
                    ),
                    Page(
                        "/verbs/doubling/distant-3", "Distant Past Tense 3: vid and ah"
                    ),
                ],
            ),
        ],
    ...
    Page(
        ...
        "Panini",
        children=[
            Page("/panini/two", "Two Easy Rules"),
            Page("/panini/structure", "The Structure of the Ashtadhyayi"),
            Page("/panini/shivasutras", "The Shiva Sutras"),
            Page("/panini/vowel", "Vowel Sandhi"),
            Page("/panini/anubandha", "The anubandha"),
        ],
    ),
]
...
@lru_cache
def ashtadhyayi_rules():
    # lru_cache won't catch file updates, but that's fine.
    path = os.path.join(_DATA_DIR, "ashtadhyayi-rules.txt")
    ret = {}
    with open(path) as f:
        for line in f:
            line = line.strip()
            if not line:
                continue
            code, _, rule = line.partition(" ")
            assert code not in ret, f"Code {code} already seen."
            ret[code] = rule
    return ret
